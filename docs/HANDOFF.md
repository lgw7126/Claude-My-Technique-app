...
3. **M3 — 전자동 파이프라인**: YouTube Data API 수집기(사용자 API 키 발급 안내 필요),
   ...
   - **중간 단계 완료**: `web/index.html`에 반자동 수집 구현 — 🔥 트렌딩 버튼.
     사용자 API 키는 localStorage 저장, 발급 가이드 내장. 다운로드만 수동으로 남음.
   - **⚠️ 데이터 소스 교체 (중요 교훈)**: 초기엔 `chart=mostPopular`(유튜브 인기 급상승)
     썼으나 결과가 음원·MV·영화/게임 트레일러 위주라 "바이럴 쇼츠"와 무관 → 사용자 불만.
     실측(사용자 키로 확인) 결과 **`search.list` (videoDuration=short, order=viewCount,
     publishedAfter=최근N일) + 상세조회(videos.list) 2단계**가 진짜 급상승 쇼츠를 반환.
     또한 **regionCode/relevanceLanguage는 약한 신호 → 결과 언어는 검색 키워드가 좌우**
     (예: q="요리"면 한국 요리 쇼츠, q="#shorts"면 인도 콘텐츠). 그래서 UI를
     **주제 키워드 검색 + 추천 칩(요리/댄스/브이로그…) + 국가/기간/길이 필터**로 재설계.
     search.list는 호출당 100 quota(하루 ~100회) → 키 입력/칩클릭/버튼에서만 호출.
   - 주의: **아티팩트/미리보기 패널은 CSP로 외부 fetch 차단** → 트렌딩은 파일로 열었을 때만
     동작 (앱 안에 안내 메시지 있음). googleapis.com 자체는 이 샌드박스에서 접근 가능 확인.
   - 사용자가 반자동 방식을 선택함 (클라우드 전자동은 네트워크 정책 해결 후 후보).
   - **v0.5 원클릭 파이프라인 (사용자 "너무 복잡" 피드백으로 추가)**:
     ① 앱 트렌딩 목록에 **⬇ 받기** 버튼 → `techniqueshorts://` 커스텀 프로토콜 호출
     ② `tools/쇼츠받기.hta` (v2) — 첫 실행 시 HKCU에 프로토콜 자동 등록(관리자 불필요),
        프로토콜로 호출되면 자동 다운로드 후 자동 닫힘. yt-dlp.exe(사용자 PC에 있음) 사용.
        수동 모드: 클립보드 링크 자동 붙여넣기 + 완료 시 explorer 열기.
        ⚠️ .hta는 다운로드 시 Windows가 차단 → 속성>차단 해제 필요 (사용자 완료함).
     ③ 앱에서 **📁 쇼츠 폴더 연결** (File System Access API, showDirectoryPicker,
        핸들은 IndexedDB에 저장) → 5초마다 폴더 스캔, 새 영상 자동 분석·등록.
     결과: 한 번 설정 후엔 트렌딩에서 ⬇ 받기 클릭 한 번이면 다운로드→분석→라이브러리 자동.
   - 사용자 PC 배치: `바탕화면\Claude\Claude-My-technique-app\` 폴더에 index.html,
     yt-dlp.exe, 쇼츠받기.hta 존재. 사용자는 비개발자·Windows·한국어. 터미널 노출 금지.
